...
uses
  ...
  Shared.FireDAC.Utils,
  REST.Backend.EMSServices;

type
  TUserProfileData = record
    DocNumber: string;
    Birthdate: string;
  end;
  ...
  TsrvUsers = class(TDataModule, IServiceGetAvatar, IServiceSendAvatar)
    ...
    mTblUserAddressaddr_cibge: TStringField;
  ...
  public
    ...
    function DoPushUserProfileData(AUserProfileData: TUserProfileData): Boolean;
  end;
...
{$REGION 'Inner methods'}
  ...
  begin
    ...
    begin
      { TODO -oRicardo Martins -cBugFix : Alterar escrita dos dados na tela em função de provável problema no LiveBindings }
      mTblUserAddressaddr_cpostalcode.AsString := LeftStr(mTblUserAddressaddr_cpostalcode.AsString, 8);
    ...
    case LValidation of
      ...
      3:
        begin
          ...
          if not LResult then
          begin
            ShowWarning(WARNING_ADDRESS_INVALID_POSTALCODE, frmUserProfile.edtTabiAddressPostalCode);
          end;
        end;
      4:
        begin
...
begin
  LTask := TTask.Create(
    ...
    begin
      ...
      LAdvJSONObjectWriter.AddStringPair('addr_cibge', mTblUserAddressaddr_cibge.AsString);
...
function TsrvUsers.DoPushUserProfileData(AUserProfileData: TUserProfileData): Boolean;
var
  dUserBirthdate: TDateTime;
begin
  Result := True;
  if not (mTblUserProfile.State in dsEditModes) then
  begin
    mTblUserProfile.Edit;
  end;

  mTblUserProfileuser_cdocnumber.AsString := OnlyNumbers(AUserProfileData.DocNumber);

  if (AUserProfileData.Birthdate <> EmptyStr) then
  begin
    if  TryStrToDateTime(AUserProfileData.Birthdate, dUserBirthdate) then
    begin
      mTblUserProfileuser_dbirthdate.AsDateTime := dUserBirthdate;
    end
    else
    begin
      Result := False;
      frmUserProfile.edtTabProfileBirthdate.Text := EmptyStr;
      ShowWarning('Data inválida', frmUserProfile.edtTabProfileBirthdate);
    end;
  end;
end;
...
//        end;
        ...
        BackEndpointPutItem.AddParameter(EMS_PUT_ITEM_PARAM_NAME_ID,
                                         mTblUserProfileuser_nid.AsString,
                                         TRESTRequestParameterKind.pkHTTPHEADER);

        BackEndpointPutItem.AddParameter(EMS_PUT_ITEM_PARAM_NAME_DATATYPE,
        ...
        try
          ...
          begin
            TThread.Synchronize(LThread,
              procedure()
              begin
                TDialogService.MessageDialog(rRespPutItem.ErrorMessage, TMsgDlgType.mtError, [TMsgDlgBtn.mbOk], TMsgDlgBtn.mbOk, 0,
                  procedure(const AResult: TModalResult)
                  begin
                    TAppSharedUtils.HideActivityIndicator;
                  end);
...
begin
  ...
  begin
    ...
    try
      ...
      if mTblUserProfileuser_cname.AsString = '' then
      begin
        mTblUserProfileuser_cname.AsString := Chronos.GetItem<string>(APP_USER_NICKNAME);
      end;
